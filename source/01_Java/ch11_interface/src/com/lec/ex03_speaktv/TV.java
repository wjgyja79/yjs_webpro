...
public class TV implements IVolume{
	...
	public void volumeUp() {
		if(volumeLevel < TV_MAX_VOLUME) {
			...
			System.out.println("TV 볼륨이 최대치(" + TV_MAX_VOLUME+")여서 올리지 못함");
		}
	}
	@Override
	public void volumeUp(int level) {
		if(volumeLevel + level < TV_MAX_VOLUME ) {
			...
			System.out.println("TV 볼륨을 " + tempLevel 
					+ "만큼 올려도 최대치("+TV_MAX_VOLUME+")입니다");
		}
	}
	@Override
	...
	public void volumeDown(int level) {
		if(volumeLevel-level > TV_MIN_VOLUME) {
			...
			System.out.println("TV 볼륨을 " + tempLevel + "만큼 내려 볼륨이 최소치(" + TV_MIN_VOLUME+")");
		} // if
	}//volumeDown
}//class