...
-- (2) ALL : 서브쿼리 결과 모두 만족하면 참
 ...
 SELECT * FROM EMP
   WHERE SAL > (SELECT SAL FROM EMP WHERE DEPTNO=30); -- 단일행 서브쿼리 이용
...
SELECT E.*, DNAME, LOC
 FROM EMP E, DEPT D
 WHERE E.DEPTNO = D.DEPTNO AND (E.DEPTNO, SAL) IN (SELECT DEPTNO, MAX(SAL) FROM EMP GROUP BY DEPTNO)
 ORDER BY E.DEPTNO;
...
SELECT HIREDATE, TRUNC(EXTRACT(MONTH FROM HIREDATE)/3)+1 FROM EMP;
...
SELECT ENAME, SAL, JOB
 FROM EMP
 WHERE SAL > ALL (SELECT SAL FROM EMP WHERE JOB = 'SALSEMAN');
 
-- 탄탄7. SALESMAN 일부 어떤 한 사원보다 급여를 많이 받는 사원들의 이름과 급여와 직급(담당 업무)를 출력하되 영업 사원도 출력(ANY)
SELECT ENAME, SAL, JOB
 FROM EMP
 WHERE SAL > ANY (SELECT SAL FROM EMP WHERE JOB = 'SALSEMAN');
...
SELECT ENAME, JOB, SAL
 FROM EMP
 WHERE SAL IN (SELECT FROM EMP WHERE ENAME = 'SCOTT' OR ENAME = 'WARD')
